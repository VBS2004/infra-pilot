...
def load_index(repo: str):
    from terra_pilot.search.index import get_index  # lazy - matches agent_tools.py
    return get_index(repo)


def build_catalog(repo: str):
    from terra_pilot.search.catalog import ModuleCatalog  # lazy
...
def hybrid_search(repo: str, query: str, *, top_k: int = 8) -> List[Dict]:
    ...
    if not query or not query.strip():
        return []
    try:
        from terra_pilot.search.lexical_search import build_hybrid  # lazy - lives in the indexer repo
...
def sibling_inputs(repo: str, component: str, *, limit: int = 2,
    ...
    """Find existing <component>/inputs.hcl (or main.tf for Plain TF) across the repo as few-shot anchors
    ...
    filename = "inputs.hcl" if convention.kind == "terragrunt" else "main.tf"
    ...
    for dirpath, _dirnames, filenames in os.walk(infra):
        ...
        if ".terragrunt-cache" in dirpath or (os.sep + "modules" + os.sep) in probe or (os.sep + ".terraform" + os.sep) in probe:
            continue
        if os.path.basename(dirpath) == component and filename in filenames:
