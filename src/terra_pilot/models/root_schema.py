...
def _parse_uncached(repo_root: str) -> RepoSchema:
    ...
    if convention.kind == "tf-modules" and convention.confidence != "low":
        return RepoSchema(
            root_file_path=None,
            root_file_name="",
            include_target="",
            module_source_template="modules/{component}",
            env_config_filename="terraform.tfvars",
            env_config_fields=["environment", "region"],
            backend_type="",
            provider_name="",
            confidence="full"
        )
    if convention.kind == "tf-flat" and convention.confidence != "low":
        return RepoSchema(
            root_file_path=None,
            root_file_name="",
            include_target="",
